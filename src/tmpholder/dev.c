...
static int nid_device_stop(struct dev_interface *);
static void __nid_device_recv_keepalive(struct dev_interface *dev_p);
static void __nid_device_recv_upgrade(struct dev_interface *dev_p, char code);
static void _sock_shutdown(struct nid_device *nid);
...
__nid_end_io(struct request *req)
{
	...
	if (error) {
		nid_log_warning("%s: %s request %p failed", log_header, req->rq_disk->disk_name, req);
	} else {
		nid_log_debug("%s: %s request %p done", log_header, req->rq_disk->disk_name, req);
	}
...
static int __nid_read_resp(struct dev_interface *dev_p)
{
...
static int
_nid_ctl_receiver(void *data)
{
	char *log_header = "_nid_ctl_receiver";
	...
	rc = nid_device_stop(dev_p);
	if (rc != 0) {
		nid_log_notice("%s: disconnecting %s ...", log_header, disk->disk_name);
	}
...
static int
_nid_dat_receiver(void *data)
{
	char *log_header = "_nid_dat_receiver";
	struct request *req;
...
/* Always call with the nid_mutex held */
static int nid_send_req(struct nid_device *nid, struct request *req)
{
	int result;
...
static int 
_nid_dat_sender(void *data)
{
	char *log_header = "_nid_dat_sender";
	struct nid_device *nid = data;
	struct gendisk *disk = nid->disk;
	struct request *req;
	int result;
...
static int
_nid_ctl_sender(void *data)
{
	char *log_header = "_nid_ctl_sender";
	int nr_reqs;
	struct nid_device *nid = data;
	struct gendisk *disk = nid->disk;
...
static int
nid_start_threads(struct dev_interface *dev_p)
{
	char *log_header = "nid_start_threads";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	struct nid_device *nid = priv_p->p_nid;
	int error = 0;

	nid->nid_ctl_snd_thread = kthread_create(_nid_ctl_sender, nid, "%s", nid->disk->disk_name);
	if (IS_ERR(nid->nid_ctl_snd_thread)) {
		struct task_struct *thread = nid->nid_ctl_snd_thread;
		nid_log(LOG_IOCTL, "%s: cannot create ctl_sender for %s\n", log_header, nid->disk->disk_name);
	...
	nid->nid_dat_snd_thread = kthread_create(_nid_dat_sender, nid, "%sds", nid->disk->disk_name);
	...
	nid->nid_ctl_rcv_thread = kthread_create(_nid_ctl_receiver, dev_p, "%scr", nid->disk->disk_name);
	...
	nid->nid_dat_rcv_thread = kthread_create(_nid_dat_receiver, nid, "%sdr", nid->disk->disk_name);
...
static int __nid_start(struct dev_interface *dev_p, struct block_device *bdev)
{
...
static int nid_stop_threads(struct nid_device *nid)
{
	/* _nid_ctl_receiver stop all other threads but itself. */
...
_sock_shutdown(struct nid_device *nid)
{
	 ...
	 * and let the _nid_ctl_receiver do cleanup work.
	 */
	if (nid->ctl_sock) {
...
static int nid_stop(struct nid_device *nid)
{
	mutex_lock(nid->nid_mutex);
	_sock_shutdown(nid);
	mutex_unlock(nid->nid_mutex);
...
static void
___nid_end_io(struct request *req)
{
	int error = req->errors ? -EIO : 0;
	struct request_queue *q = req->q;
	unsigned long flags;
	...
	nid = (struct nid_device *)req->rq_disk->private_data;
	if (error) {
		nid_log_warning("___nid_end_io: %s request %p failed", req->rq_disk->disk_name, req);
	} else {
		nid_log_debug("___nid_end_io: %s request %p done", req->rq_disk->disk_name, req);
	}
...
static void
__driver_to_recover(struct dev_interface *dev_p)
{
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	...
	nid_log_debug("_driver_to_recover: minor:%d", priv_p->p_minor);	
	msg_buf = kmalloc(128, GFP_KERNEL);
	len = 128;
	memset(&nid_msg, 0, sizeof(nid_msg));
	nid_msg.m_req = NID_REQ_RECOVER;
	nid_msg.m_devminor = priv_p->p_minor;
	nid_msg.m_has_devminor = 1;
	drv_p->dr_op->dr_encode_message(drv_p, msg_buf, &len, &nid_msg);
	drv_p->dr_op->dr_send_message(drv_p, msg_buf, len);
}

static void
__driver_established(struct dev_interface *dev_p)
{
	char *log_header = "__driver_established";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	struct drv_interface *drv_p = priv_p->p_drv;
       	char *msg_buf;
	uint32_t len = 0;
	...
	nid_log_notice("%s: device minor:%d", log_header, priv_p->p_minor);	
	msg_buf = kmalloc(128, GFP_KERNEL);
	len = 128;
	memset(&nid_msg, 0, sizeof(nid_msg));
...
static void
__device_deleted(struct dev_interface *dev_p)
{
	char *log_header = "__device_deleted";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	struct drv_interface *drv_p = priv_p->p_drv;
...
static int
_nid_recover(struct dev_interface *dev_p, struct block_device *bdev)
{
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	struct nid_device *nid = priv_p->p_nid;
	int error = 0;
 ...
 */
static void
dev_nid_to_recover(struct dev_interface *dev_p)
{
	char *log_header = "dev_nid_to_recover";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	int minor = priv_p->p_minor;
...
static int
nid_device_stop(struct dev_interface *dev_p)
{
...
static void
__nid_device_recv_keepalive(struct dev_interface *dev_p)
{
	char *log_header = "__nid_device_recv_keepalive";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	struct drv_interface *drv_p = priv_p->p_drv;
	struct nid_device *nid = priv_p->p_nid;
       	char *msg_buf;
	...
	nid_log_debug("%s: got keepalive from nidservice for %s", log_header, nid->disk->disk_name);	
	msg_buf = kmalloc(128, GFP_KERNEL);
	memset(&nid_msg, 0, sizeof(nid_msg));
...
static void
__nid_device_recv_upgrade(struct dev_interface *dev_p, char code)
{
	char *log_header = "__nid_device_recv_upgrade";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	struct drv_interface *drv_p = priv_p->p_drv;
	struct nid_device *nid = priv_p->p_nid;
       	char *msg_buf;
	...
	nid_log_debug("%s: for %s, code: %d", log_header, nid->disk->disk_name, code);
	msg_buf = kmalloc(128, GFP_KERNEL);
	memset(&nid_msg, 0, sizeof(nid_msg));
...
static int
dev_nid_init(struct dev_interface *dev_p, uint64_t size, uint32_t blksize)
{
	char *log_header = "dev_nid_init";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	int minor = priv_p->p_minor;
	struct block_device *bdev;
	struct nid_device *nid;

	nid_log_error("%s: (minor:%d, size:%llu, blksize:%u): start ...",
...
static int 
dev_nid_start(struct dev_interface *dev_p, int rsfd, int dsfd)
{
	char *log_header = "dev_nid_start"; 
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	int minor = priv_p->p_minor;
	struct block_device *bdev;
	struct nid_device *nid;
	...
	nid_log_error("%s: (minor:%d, rsfd:%d, dsfd:%d): start...", log_header, minor, rsfd, dsfd);
	bdev = _nid_bdget(minor);
	if (!bdev) {
		nid_log_error("%s: (minor:%d): got null bdev", log_header, minor);
		//bdput(bdev);
...
dev_nid_to_delete(struct dev_interface *dev_p)
{
	...
	if (!nid->data_sock) {
		...
		nid->disconnect = 1;
		/* _nid_ctl_receiver is not there, we must cleanup by ourself. */
		nid_device_stop(dev_p);
		bdput(bdev);
		return;
	}
	fsync_bdev(bdev);
...
static int
dev_nid_send_upgrade(struct dev_interface *dev_p, char force)
{
	char *log_header = "dev_nid_send_upgrade";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	int minor = priv_p->p_minor;
	struct block_device *bdev;
	struct nid_device *nid;
...
static int
dev_nid_send_keepalive(struct dev_interface *dev_p)
{
	char *log_header = "dev_nid_send_keepalive";
	struct dev_private *priv_p = (struct dev_private *)dev_p->dv_private;
	int minor = priv_p->p_minor; 
	struct block_device *bdev;
	struct nid_device *nid;
		...
		__releases(q->queue_lock) __acquires(q->queue_lock)
{
	...
	while ((req = blk_fetch_request(q)) != NULL) {
		...
		if (unlikely(nid->error_now)) { 
			...
			spin_unlock_irq(&nid->queue_lock);
			req->errors++;
			nid_log_warning("%s: disk:%s is error_now", log_header, req->rq_disk->disk_name);
			__nid_end_io(req);
...
static struct dev_operations dev_op = {
	.dv_nid_init = dev_nid_init,
	.dv_nid_start = dev_nid_start,
	.dv_nid_to_delete = dev_nid_to_delete,
	.dv_nid_set_ioerror = dev_nid_set_ioerror,
	.dv_nid_send_upgrade = dev_nid_send_upgrade,
	.dv_nid_to_recover = dev_nid_to_recover,
	.dv_nid_send_keepalive = dev_nid_send_keepalive,
...
dev_initialization(struct dev_interface *dev_p, struct dev_setup *setup)
{
	...
	priv_p->p_nid = kcalloc(1, sizeof(*nid_dev), GFP_KERNEL);
