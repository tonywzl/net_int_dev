...
#define RAID_COMMAND	"python /opt/milio/atlas/roles/pool/cp-load.pyc"
...
get_one_message(int sfd, char *buf)
{
	...
	if (nread < 0) {
		nid_log_error("get_one_message: fd:%d read_n error", sfd);
		return -1;
	}
	...
	nid_log_debug("get_one_message: got cmd:%d, len:%d, msg:{%s}",
		cmd, msg_len, p);
	return 0;
}
...
make_request_connection(char *ipstr, u_short port)
{
	...
	if (error) {
		//nid_log_error("getsockopt connection error: %d.", error);
		close(sfd);		/* just in case */
		errno = error;
		return -1;
	}

	write(sfd, &chan_type, 2);
	return sfd;
}
...
make_data_connection(u_short port)
{
	...
	sfd = socket(AF_INET, SOCK_STREAM, 0);
	if (sfd < 0) {
		nid_log_error("make_data_connection: cannot open socket, errno:%d", errno);
		return -1;
	}
...
static int
channel_establish(struct ash_interface *ash_p, char *uuid, char *server_ipstr,
	u_short port, int *data_sfd, uint64_t *size)
{
	...
	len = ash_p->h_op->h_exp_generator(ash_p, uuid, buf);
	...
	/* setup size */
	if (read_n(rsfd, &size_h, 4) < 0 ||
	    read_n(rsfd, &size_l, 4) < 0) {
		nid_log_debug("channel_establish: fd:%d read_n error", rsfd);
		close(rsfd);
		return -1;
	}
	...
	*size = exp_size;
	...
	nid_log_debug("channel_establish: make_data_connection successful at port:%d", port);
	clen = sizeof(caddr);
	...
	nid_log_info("client: got connect from the server, fd:%d", dsfd);
	*data_sfd = dsfd;
	close(a_sfd);
...
static int
raid1_device_set_io_error(char *uuid)
{
	FILE *fp;
	char cmd_buf[2048], buff[4096];
	int rc;

	sprintf(cmd_buf, "%s raid1_device_set_io_error %s", RAID_COMMAND, uuid);
	fp = popen(cmd_buf, "r");
	while (fgets(buff, sizeof(buff), fp) != NULL) {
		nid_log_debug("%s", buff);
	}
...
static int
raid5_device_set_io_error(char *uuid)
{
	FILE *fp;
	char cmd_buf[2048], buff[4096];
	int rc;
	sprintf(cmd_buf, "%s raid5_device_set_io_error %s", RAID_COMMAND, uuid);
	fp = popen(cmd_buf, "r");
	while (fgets(buff, sizeof(buff), fp) != NULL) {
		nid_log_debug("%s", buff);
	}
...
static int
raid1_device_unset_io_error(char *uuid)
{
	char cmd_buf[2048];
	sprintf(cmd_buf, "%s raid1_device_unset_io_error %s", RAID_COMMAND, uuid);
	return system(cmd_buf);
}

static int
raid5_device_unset_io_error(char *uuid)
{
	char cmd_buf[2048];
	sprintf(cmd_buf, "%s raid5_device_unset_io_error %s", RAID_COMMAND, uuid);
	return system(cmd_buf);
}

static int
raid1_set_device_faulty(char *uuid)
{
	FILE *fp;
	char cmd_buf[2048], buff[4096];
	int rc;

	sprintf(cmd_buf, "%s raid1_set_device_faulty %s", RAID_COMMAND, uuid);
	fp = popen(cmd_buf, "r");
	while (fgets(buff, sizeof(buff), fp) != NULL) {
		nid_log_debug("%s", buff);
	}
...
int main(int argc, char *argv[]) {
	...
	ash_p = calloc(1, sizeof(*ash_p));
	ash_initialization(ash_p, &ash_setup);
	req_sfd = channel_establish(ash_p, uuid, server_ipstr, port, &data_sfd, &size64);
	if (req_sfd < 0) {
		nid_log_error("cnanot establish channel to the server, %s:%d",
			server_ipstr, port);
		exit(1);
	}
	...
	finish_sock(req_sfd, data_sfd, nbd);
	...
	do {
		...
		if ((rc=ioctl(nbd, NBD_DO_IT)) < 0) {
			int error = errno;
			int hold_raid1_io_error_lock = 0;
			int hold_raid5_io_error_lock = 0;
				...
				if(cont) {
					...
					for (;;) {
						...
						if (io_counter > 0) {
							...
							io_counter = -1;
							if (!raid1_device_set_io_error(uuid)) {
								nid_log_debug("success to do raid1_device_set_io_error for %s", uuid);
								hold_raid1_io_error_lock = 1;
							} else if (!raid5_device_set_io_error(uuid)) {
								nid_log_debug("failed to do raid1_device_set_io_error for %s", uuid);
								nid_log_debug("success to do raid5_device_set_io_error for %s", uuid);
								hold_raid5_io_error_lock = 1;
							} else {
								nid_log_debug("failed to do raid1_device_set_io_error for %s", uuid);
								nid_log_debug("failed to do raid5_device_set_io_error for %s", uuid);
								continue;
							}
					...
					finish_sock(req_sfd, data_sfd, nbd);
					if (hold_raid1_io_error_lock) {
						nid_log_debug("doing raid1_device_unset_io_error for %s", uuid);
						raid1_device_unset_io_error(uuid);
						hold_raid1_io_error_lock = 0;
					} else if (hold_raid5_io_error_lock) {
						nid_log_debug("doing raid5_device_unset_io_error for %s", uuid);
						raid5_device_unset_io_error(uuid);
						hold_raid5_io_error_lock = 0;
					}
