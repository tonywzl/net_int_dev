...
static int d_cxt_reqsz = UMSG_TX_HEADER_LEN;

struct ini_key_desc cxt_keys[] = {
	{"type",		KEY_TYPE_STRING,	"cxt",			1,	DESCRIPTION_TYPE},
	{"req_size",            KEY_TYPE_INT,           &d_cxt_reqsz,           0,	DESCRIPTION_REQ_SIZE},
	{"ds_name",             KEY_TYPE_STRING,        "wrong_name",           0,	DESCRIPTION_DS_NAME},
	{NULL,			KEY_TYPE_WRONG,		NULL,			0,	NULL}
};

void
conf_cxt_registration(struct list_head *key_set_head, struct lstn_interface *lstn_p)
{
	struct list_node *lnp;

	lnp = lstn_p->ln_op->ln_get_node(lstn_p);
	lnp->ln_data = (void *)cxt_keys;
