...
#define list_del(ent)\
do {\
	struct list_head *entry = ent;\
	nid_log_debug("list_del() entry:%p, prev:%p, next:%p", entry, entry->prev, entry->next);\
	__list_del(entry->prev, entry->next);\
	entry->next = LIST_POISON1;\
	entry->prev = LIST_POISON2;\
} while(0)
 ...
 */
#define list_for_each_entry_safe_continue(pos, type, n, type2, head, member) 		\
	for (pos = list_entry(pos->member.next, type, member), 		\
		n = list_entry(pos->member.next, type, member);		\
	     &pos->member != (head);						\
	     pos = n, n = list_entry(n->member.next, type2, member))
 ...
 */
#define list_for_each_entry_safe_from(pos, type, n, type2, head, member) 			\
 ...
 * of list entry.
 */
#define list_for_each_entry_safe_reverse(pos, type, n, type2, head, member)		\
	for (pos = list_entry((head)->prev, type, member),	\
		n = list_entry(pos->member.prev, type, member);	\
	     &pos->member != (head); 					\
	     pos = n, n = list_entry(n->member.prev, type2, member))
 ...
 * @member:	the name of the hlist_node within the struct.
 */
#define hlist_for_each_entry_continue(tpos, type, pos, member)		 \
	for (pos = (pos)->next;						 \
	     pos && ({ prefetch(pos->next); 1;}) &&			 \
		({ tpos = hlist_entry(pos, type, member); 1;}); \
	     pos = pos->next)
 ...
 * @member:	the name of the hlist_node within the struct.
 */
#define hlist_for_each_entry_from(tpos, type, pos, member)			 \
