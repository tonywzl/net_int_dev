...
main()
{
	...
	if(!dxa_p) {
		nid_log_error("failed to create dxa channel");
		goto out;
	}
	int ret = dxa_p->dx_op->dx_start(dxa_p);
	if(ret) {
		nid_log_error("failed to start dxa");
		goto cleanup;
	}

	nid_log_warning("success start dxa, begin send out message test");
	...
	// wait some time to ensure dxt already running
	sleep(3);
	while(1) {
