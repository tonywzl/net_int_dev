/**
 * UT description:
 * 1, Random generate UT_BWC_REQ_NUM write requests, and write the request data to ut_bwc_write_data_dev;
...
#define UT_BWC_REQ_NUM 			10000
#define UT_MRW_REPEAT_WRITE_TIME	1
...
ut_bwc_gen_data()
{
	...
	for (i=0; i<ut_bwc_write_req_num; i++) {
		...
		write(fd, ut_bwc_req[i].buf, ut_bwc_req[i].len);
...
ut_bwc_init_env ()
{
	...
	strcpy(tp_setup.name, "BWC_UT1_TP");
...
main()
{
	...
	int i;
	for (i=0; i<UT_MRW_REPEAT_WRITE_TIME; i++) {
		ut_bwc_write_data();
		usleep(1000);
	}
