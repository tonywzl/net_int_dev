/**
 * UT description:
 * 1, Prepare some pages with continues offsets, and use those pages generate UT_BWC_REQ_NUM write requests,
 * 2, Write the request data to ut_bwc_write_data_dev;
 * 3, Call bwc->write_list function send those requests, go through the bwc module then write to /dev/sdb;
 * 4, Check /dev/sdb data and ut_bwc_write_data_dev data if match or not.
 */
...
#define UT_BWC_UUID_BWC			"UUID_BWC_123"
#define UT_BWC_WC_PPAGE_SZ		8
#define UT_BWC_SEG_SZ 			1044480
#define UT_BWC_MAX_OFFSET 		(1024 * 1024 * 10L)
...
volatile int 				ut_bwc_write_req_num = 0;
...
struct ut_bwc_page_blocks		ut_bwc_cross_page_offset[] = {
		...
		{
				.offset_num = 6,
				.offsets = {40, 60, 90, 91, 161, 194},
				.lens = {20, 30, 1, 70, 33, 110}
		},
		{
				.offset_num = 2,
				.offsets = {11, 20},
...
ut_bwc_gen_data()
{
	...
	ut_bwc_req = malloc(sizeof(*ut_bwc_req) * ut_bwc_write_req_num);
	k = 0;
	for (i=0; i<ut_bwc_write_total_cross_page; i++) {
		pglen = 0;
		for (j=0; j<ut_bwc_cross_page_offset[i].offset_num; j++) {
			ut_bwc_req[k].off = (1024 * 1024) + (((off_t)ut_bwc_cross_page_offset[i].offsets[j]) << 12);
			assert(ut_bwc_req[k].off < UT_BWC_MAX_OFFSET);
			ut_bwc_req[k].len = ((uint32_t)ut_bwc_cross_page_offset[i].lens[j]) << 12;
			pglen += ut_bwc_req[k].len;
			ut_bwc_req[k].buf = malloc(ut_bwc_req[k].len);
			ut_bwc_gen_random_data(ut_bwc_req[k].buf, ut_bwc_req[k].len);
			ut_bwc_cross_page_offset[i].buffer[i] = ut_bwc_req[k].buf;
			lseek(fd, ut_bwc_req[k].off,  SEEK_SET);
			write(fd, ut_bwc_req[k].buf, ut_bwc_req[k].len);
			total += ut_bwc_req[k].len;
			printf("Generate request:index:%d offset:%ld length:%u\n",
					k, ut_bwc_req[k].off, ut_bwc_req[k].len);
			fflush(stdout);
			k++;
...
ut_bwc_init_env ()
{
	...
	strcpy(tp_setup.name, "BWC_UT4_TP");
	...
	ut_bwc_wc_handle = ut_bwc_wc.wc_op->wc_create_channel(&ut_bwc_wc, &ut_bwc_sac, &wc_info, UT_BWC_UUID_BWC, &new_worker);
...
main()
{
	...
	printf("wait for flush to disk start\n");
	while(1) {
