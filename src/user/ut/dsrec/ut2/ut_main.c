...
dsmgr_put_space(struct dsmgr_interface *dsmgr_p, struct block_size_node *np)
{
  ...
  assert( np->bsn_index < 4294967000 );
  	list_add_tail( &np->bsn_list, &dsmgr_list_head );
...
main()
{
	...
	*rc_pop = rc_op1;
	...
	while  ( i < TEST_POOL_SIZE ) {
		...
		np = p_blksn->bsn_op->bsn_get_node( p_blksn );
		np->bsn_index = i;
		np->bsn_size = 1;
		list_add_tail( &np->bsn_list, &list_nodes_to_reclaim );
