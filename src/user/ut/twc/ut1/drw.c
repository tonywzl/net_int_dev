...
static void *
drw_create_worker(struct drw_interface *drw_p, char *exportname, int rw_sync, int direct_io)
{
...
next_try:
	...
	while (!list_empty(&priv_p->p_write_op_head)) {
		...
		sleep(1);
		pthread_mutex_lock(&priv_p->p_wlck);
		__sync_sub_and_fetch(&ut_twc_flush_left, 1);
	}
