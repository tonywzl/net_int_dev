...
struct list_head* get_and_verify_space_from_dsmgr()
{
	...
	struct list_head* list_dsmgr_space_size_ptr;
...
struct list_head* traverse_dsbmp_rtree()
{
  //struct dsmgr_private* priv_p = (struct dsmgr_private* ) ut_dsmgr.dm_private;
	//  	struct dsbmp_interface dsbmp = (struct dsbmp_interface)priv_p->p_dsbmp;
  	struct list_head* rtree_list = NULL;

	rtree_list = ut_dsmgr.dm_op->dm_traverse_dsbmp_rtree( &ut_dsmgr );
	// rtree_list = dsbmp.sb_op->sb_traverse_rtree( &dsbmp);
	return rtree_list;
