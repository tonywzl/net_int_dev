...
struct list_head* get_and_verify_space_from_dsmgr()
{
	...
	list_for_each_entry( cur_np, struct block_size_node, list_dsmgr_space_size_ptr, bsn_list ) {
	  	nid_log_debug( "dsmgr_unit - get_and_verify..() size = %lu, offset = %lu\n", cur_np->bsn_size, cur_np->bsn_index );
...
void add_all_space_back_to_dsmgr()
{
	...
	nid_log_debug( "dsmgr_unit -  add_all_space_back_to_dsmgr()\n" );
	int iteration = 0;
	list_for_each_entry_safe( bn,  temp_bn, struct block_size_node, &list_used_blocks_size, bsn_list ) {
	  	nid_log_debug( "dsmgr_unit - 40k block,  the size of the block is %lu, offset is %lu, iteration = %d\n",  bn->bsn_size, bn->bsn_index, iteration );
...
{
  	struct block_size_node *bn, *temp_bn;
	nid_log_debug( "dsmgr_unit - put_space_to_dsmgr()\n" );
	// uint64_t size;
	...
	nid_log_debug( "dsmgr_unit - put_space_to_dsmgr() - size_needed = %lu\n", (unsigned long)size_needed );

	int iteration = 0;
	int found_block = 0;

	list_for_each_entry_safe( bn,  temp_bn, struct block_size_node, &list_used_blocks_size, bsn_list ) {
	  	nid_log_debug( "dsmgr_unit - the size of the block is %lu, offset is %lu, iteration = %d\n",  bn->bsn_size, bn->bsn_index, iteration );
		if ( ( size_needed > 0 ) && ( ! found_block ) ) {
	  		if ( size_needed < bn->bsn_size ) {
			  	nid_log_debug( "dsmgr_unit - put_space_to_dsmgr() - putting  block size is %lu, offset is %lu\n",  bn->bsn_size, bn->bsn_index );
				list_del( &bn->bsn_list );
				list_del( &bn->bsn_olist );
				ut_dsmgr.dm_op->dm_put_space( &ut_dsmgr, bn);
	...
	if ( ! found_block ) {
	  	list_for_each_entry_safe_reverse( bn,  struct block_size_node, temp_bn, struct block_size_node, &list_used_blocks_size, bsn_list ) {
		  	if ( ( size_needed > 0 ) && ( ! found_block ) ) {
			  	nid_log_debug( "dsmgr_unit - put_space_to_dsmgr() - putting  block size is %lu, offset is %lu, size_needed = %lu\n",  bn->bsn_size, bn->bsn_index, size_needed );
...
void get_space_from_dsmgr( uint64_t max_get_size )
{
	...
	orig_size = size;
	nid_log_debug( "dsmgr_unit - get_space_from_dsmgr - size is %lu\n", (unsigned long) size );
	...
	list_for_each_entry_safe( bn, temp_bn, struct block_size_node, &out_list1, bsn_list ) {
		...
		uint64_t prev_size_index = 0;
	  	...
	  	list_for_each_entry_safe( bn2, temp_bn2, struct block_size_node, &list_used_blocks_size, bsn_list ) {
			  ...
			  if ( list_size_2_prev_marker ) {
			  	nid_log_debug( "dsmgr_unit - add to size list  new block size is %lu, offset is %lu, adding before block w/size %lu, after block w/size %lu\n",  bn->bsn_size, bn->bsn_index, bn2->bsn_size, prev_size_index );
				//list_del( &bn->bsn_list );
				...
				nid_log_debug( "dsmgr_unit - add to beginning of size list  new block size is %lu, offset is %lu, adding before block w/index %lu\n",  bn->bsn_size, bn->bsn_index, bn2->bsn_index );
		...
		if ( ! size_list_added ) {
			...
			nid_log_debug( "dsmgr_unit - add to size list - last -  block size is %lu, offset is %lu\n",  bn->bsn_size, bn->bsn_index );
		}
		...
		uint64_t prev_offset_index = 0;
	  	list_for_each_entry_safe( bn3, temp_bn3, struct block_size_node, &list_used_blocks_offset, bsn_olist ) {
			  if ( ( list_used_blocks_offset.next == NULL ) || ( list_used_blocks_offset.next->next == NULL ) ) {
			  	nid_log_crit( "list_used_blocks_offset has a NULL for one of its pointers\n" );
				assert( 0 );
		  	}
		  	if ( bn3->bsn_index > bn->bsn_index ) {
			  	...
			  	if ( list_offset_2_prev_marker ) {
				  	nid_log_debug( "dsmgr_unit - add to offset list  new block size is %lu, offset is %lu, adding after block w/index %lu\n",  bn->bsn_size, bn->bsn_index, prev_offset_index );
			  		list_add( &bn->bsn_olist, list_offset_2_prev_marker );
					offset_list_added = 1;
			  		break;
			  	} else { // need to add to beginning of the list
					nid_log_debug( "dsmgr_unit - add to beginning of offset list  new block size is %lu, offset is %lu, adding before block w/index %lu\n",  bn->bsn_size, bn->bsn_index, bn3->bsn_index );
		...
		if ( ! offset_list_added )  {
			...
			nid_log_debug( "dsmgr_unit - add to offset list - last-  block size is %lu, offset is %lu - prev index = %lu\n",  bn->bsn_size, bn->bsn_index, prev_offset_index );
		}
	}
	...
	if ( size < orig_size ) {
        ...
        nid_log_debug( "dsmgr_unit - get_space_from_dsmgr() - total_size_retrieved = %lu\n", total_size_retrieved );
	...
	list_2_marker = &np2->bsn_olist;
	  ...
	  if ( list1_behind_list2 ) {
	    ...
	    if ( np2->bsn_index < np1->bsn_index ) {
	    	...
	    	nid_log_debug( "dsmgr_unit - list2 has index - %lu - which did not match in list1\n", np2->bsn_index );
	    	list1_behind_list2 = 0;
	    } else if ( np2->bsn_index == np1->bsn_index ) {
	    	// there is a match - this is bad - print error and exit
	    	nid_log_debug( "dsmgr_unit - contrast_dsbmp_rtree_lists() - match found with bsn_index %lu\n", np2->bsn_index );
	  ...
	  if ( ! list1_behind_list2 ) {
		...
		break;
	    	...
	    	if ( &np2->bsn_olist == list_2_marker ) {
	  		nid_log_debug( "dsmgr_unit - iterated through list2\n" );
		} else {
		  list_2_marker = list_2_marker_temp;
		}

	  }
	  nid_log_debug( "dsmgr_unit - moving past list1 entry - that does not match with index: %lu\n", np1->bsn_index );
	}
	if ( &np1->bsn_olist == list_1 ) {
	  	nid_log_debug( "dsmgr_unit - iterated through list1\n" );
	  	if (num_np2_comparisons < 3 ) {
	    		nid_log_debug( "dsmgr_unit - iterate through list 2\n" );
	    		list_for_each_entry( np2, struct block_size_node, list_2, bsn_olist ) {
	      			nid_log_debug( "dsmgr_unit - list 2 entry has index %lu\n", np2->bsn_index );
	    		}
	  	}
	}
	nid_log_debug( "dsmgr_unit - no matches\n" );
	return 0;
}
...
int contrast_offsets_of_space_lists( struct list_head* list_1, uint64_t size, uint64_t offset ) 
{
	...
	printf( "contrast_offsets_of_space_lists() - size = %lu,offset = %lu\n", size, offset );
  	list_for_each_entry( cur_np, struct block_size_node, list_1, bsn_list ) {
	  	nid_log_debug( "dsmgr_unit - contrast_offsets..() - cur_np - offset = %lu, size = %lu\n", cur_np->bsn_index, cur_np->bsn_size );
	  	if ( ( cur_np->bsn_size == size ) && ( cur_np->bsn_index <= offset ) ) {
		  	if ( cur_np->bsn_index == offset ) { // assume offsets are sorted so no need to go through
		    		nid_log_debug( "dsmgr_unit - match found on size and offset of np2 and np1 - offset = %lu, size = %lu\n", cur_np->bsn_index, cur_np->bsn_size );
		    		exit( 1 );
		  	} else {
		    		nid_log_debug( "dsmgr_unit - contrast_offsets..() - moving past list_1 element - of offset %lu, size %lu\n", cur_np->bsn_size, cur_np->bsn_index );
		  	}
		} else {
...
int contrast_dsmgr_space_lists( struct list_head* list_1, struct list_head* list_2 ) 
{
	...
	// list_2_marker = &np2->bsn_list;
  	...
  	list_for_each_entry( np1, struct block_size_node, list_1   , bsn_list ) {
	  	...
	  	nid_log_debug( "dsmgr_unit - contrast_dsmgr_space - np1 -next entry - size = %lu, offset = %lu\n", np1->bsn_size, np1->bsn_index );
	  	if ( list1_behind_list2 ) {
	    		// do not advance list2 marker - because need to catch up
	    		if ( np2->bsn_size < np1->bsn_size ) {
	      			// need to iterate to next entry of list2 - no match - unset list1_behind_list2
	      			nid_log_debug( "dsmgr_unit - list2 has size - %lu - which did not match in list1\n", np2->bsn_size );
	      			list1_behind_list2 = 0;
	    		} else if ( np2->bsn_size == np1->bsn_size ) {
	      			// there is a match - on the size - need to check index
	      			nid_log_debug( "dsmgr_unit - contrast_dsbmp_rtree_lists() - match found with bsn_size %lu, check offsets\n", np2->bsn_size );
				if ( np2->bsn_index == np1->bsn_index ) {
	  	...
	  	if ( ! list1_behind_list2 ) {
		  	if ( ! list_2_marker ) {
				...
				nid_log_debug( "dsmgr_unit - contrast_dsmgr_space - np2 -next entry - size = %lu, offset = %lu\n", np2->bsn_size, np2->bsn_index );
	      			if ( np2->bsn_size < np1->bsn_size ) {
					// need to iterate to next entry of list_2 - no match 
	      				nid_log_debug( "dsmgr_unit - list2 has index - %lu - which did not match in list1\n", np2->bsn_index );
	      			} else if ( np2->bsn_size == np1->bsn_size ) {
					// there is a match, print error and exit - we don't want a match here
					nid_log_debug( "dsmgr_unit - found  a match at size %lu - checking indexes and of other entries\n", np2->bsn_size );
					if ( np2->bsn_index == np1->bsn_index ) {
					  	nid_log_error( "match found on size and offset of np2 and np1 - offset = %lu, size = %lu\n", np2->bsn_index, np2->bsn_size );
						exit( 0 );
					} else {
	  	...
	  	nid_log_debug( "dsmgr_unit - moving past list1 entry w/size %lu - that does not match list2 entry with size: %lu\n", np1->bsn_size, np2->bsn_size );
	}
	if ( &np1->bsn_list == list_1 ) {
	  	...
	  	if (num_np2_comparisons < 3 ) {
	    		...
	    		list_for_each_entry( np2, struct block_size_node, list_2, bsn_list ) {
	      			nid_log_debug( "dsmgr_unit - list 2 entry has size %lu, index %lu\n", np2->bsn_size, np2->bsn_index );
	    		}
	  	}
	}
...
int compare_dsbmp_rtree_lists( struct list_head* list_1, struct list_head* list_2 )
{
	...
	int first_element_list_2 = 1;
	  ...
	  if  ( ( np2->bsn_size == np1->bsn_size ) && (np2->bsn_index == np1->bsn_index ) ) {
	    nid_log_info( "match on node of size %lu and index %lu\n", np2->bsn_size, np2->bsn_index );
	  } else {
	    nid_log_info(  " no match on node of size %lu and index %lu\n", np2->bsn_size, np2->bsn_index );
	    return 0;
	  }
  	}
...
void get_small_chunks_add_big_chunks( uint64_t put_max_size, uint64_t get_max_size  )
{
	...
	nid_log_info( "dsmgr_unit - get_small_chunks_add_big_chunks()\n" );
...
void get_big_chunks_add_small_chunks( uint64_t put_max_size, uint64_t get_max_size  )
{
	...
	nid_log_info( "dsmgr_unit - get_big_chunks_add_small_chunks()\n" );
...
void print_num_free_blocks()
{
  ...
  nid_log_info( "dsmgr_unit() - the number of free blocks is %lu\n", num_free_blocks );
...
{
	nid_log_open();
	nid_log_set_level( 10 );
	nid_log_debug("dsmgr ut debug module start ...");
	nid_log_set_level( 10 );
	//uint64_t* p_bitmap = calloc( 100000, 64 );

	initialize_dsmgr();
	nid_log_debug( "dsmgr ut - after initialize_dsmgr()\n" );
	get_and_verify_space_from_dsmgr();
