...
void get_initial_space_from_dsmgr()
{
  // struct list_head  out_list3;
	...
	while ( i < 5 ) {
		...
		list_splice( &out_list1, &list_used_blocks_size );
		i ++;
	...
	while ( i < 5 ) {
		...
		i ++;
		list_splice( &out_list2, &list_used_blocks_size );
	...
	while ( i < 10 ) {
		...
		i ++;
		list_splice( &out_list3, &list_used_blocks_size );

	}

	nid_log_debug( "listing all of the used blocks\n" );
	list_for_each_entry( bn,  struct block_size_node, &list_used_blocks_size, bsn_list ) {
