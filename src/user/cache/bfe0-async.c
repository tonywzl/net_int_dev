...
__rw_callback(int errorcode, struct rw_callback_arg *arg)
{
	...
	if (arg->ag_type == RW_CB_TYPE_ALIGNMENT) {
		...
		tfn = fan->d1_data;
		priv_p = (struct bfe0_private *)tfn->d2_data[BFE_PRIV_INDEX];
	...
	if (errorcode) {
		...
		if (fan->d1_retry_time++ < priv_p->p_write_max_retry_time) {
			...
			if (do_fp) {
				...
				rw_p->rw_op->rw_pwritev_async_fp(rw_p, fan->d1_rw_handle, fan->d1_iov,
					fan->d1_counter, fan->d1_offset, __rw_callback, &fan->d1_arg);
			} else {
				rw_p->rw_op->rw_pwritev_async(rw_p, fan->d1_rw_handle, fan->d1_iov,
						fan->d1_counter, fan->d1_offset, __rw_callback, &fan->d1_arg);
			}
			return;
		} else {
...
inline static void
__update_seq_flushed(struct bfe0_private *priv_p, uint64_t seq)
{
 ...
 */
static void
__extend_write_request_async(struct bfe0_private *priv_p, struct data_description_node *dnp, struct d2c_node *tfn, char is_update_fl_cnt)
{
	...
	if (!priv_p->p_do_coalescing || tfn->d2_partial_flush) {
		...
		if (!(fan = (struct d1a_node *)dnp->d_callback_arg)) {
			...
			/*
			 * Already been set by __bfe0_overwritten_wait
			 */
			fan->d1_data = (void *)tfn;
		}
		dnp->d_flushed = 1;
	...
	while (fan->d1_counter < BFE_MAX_IOV && (cur_dnp = bse_p->bs_op->bs_traverse_next_async(bse_p))) {
		...
		if (priv_p->p_ssd_mode) {
			...
			if (data_buf_len + cur_dnp->d_flush_len > priv_p->p_flush_pp_page_size) {
...
write_out:
	...
	fan->d1_rw_handle = dnp->d_rw_handle;
	fan->d1_retry_time = 0;
	priv_p->p_flush_counter++;
	if (do_fp) {
		rw_p->rw_op->rw_pwritev_async_fp(rw_p, fan->d1_rw_handle, fan->d1_iov,
 ...
 */
static void
__bfe0_overwritten_wait(struct bfe0_private *priv_p, struct data_description_node *np,
...
static inline int
__bfe0_pre_finish_flush(struct bfe0_private *priv_p, struct pp_page_node *pnp)
{
 ...
 */
static void
__bfe0_finish_flush(struct bfe0_private *priv_p, struct pp_page_node *pnp)
{
	char *log_header = "__bfe0_finish_flush";
 ...
 */
static void
bfe0_flush_page(struct bfe_interface *bfe0_p, struct pp_page_node *pnp)
{
	char *log_header = "bfe0_flush_page";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	struct d2cn_interface *d2cn_p = &priv_p->p_d2cn;
	struct d2c_node *tfn;
 ...
 */
static int
bfe0_right_extend_node(struct bfe_interface *bfe0_p, struct data_description_node *np,
	struct data_description_node *new_np)
{
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	int rc = 1;
	...
	if (!np->d_flushing) {
		...
		if (new_np->d_flush_overwritten) {
			 ...
			 */
			__bfe0_overwritten_wait(priv_p, new_np, np);
 ...
 */
static void
bfe0_add_node(struct bfe_interface *bfe0_p, struct data_description_node *new_np)
{
	char *log_header = "bfe0_add_node";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	uint32_t page_sz = priv_p->p_pagesz;
 ...
 */
static void
bfe0_left_cut_node(struct bfe_interface *bfe0_p, struct data_description_node *np,
	uint32_t cut_len, struct data_description_node *new_np)
{
	char *log_header = "bfe0_left_cut_node";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	uint32_t page_sz = priv_p->p_pagesz;
	...
	if (np->d_flushing && !np->d_flushed_back) {
		 ...
		 */
		__bfe0_overwritten_wait(priv_p, np, new_np);
 ...
 */
static void
bfe0_right_cut_node(struct bfe_interface *bfe0_p, struct data_description_node *np,
	uint32_t cut_len, struct data_description_node *new_np)
{
	char *log_header = "bfe0_right_cut_node";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	uint32_t page_sz = priv_p->p_pagesz;
	struct pp_page_node *pnp = np->d_page_node;
 ...
 */
static int
bfe0_right_cut_node_adjust(struct bfe_interface *bfe0_p, struct data_description_node *np,
	uint32_t cut_len, struct data_description_node *a_np, struct data_description_node *new_np)
{
	char *log_header = "bfe0_right_cut_node_adjust";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	uint32_t page_sz = priv_p->p_pagesz;
	struct pp_page_node *pnp = np->d_page_node;
	...
	if (np->d_flushing && !np->d_flushed_back) {
		 ...
		 */
		__bfe0_overwritten_wait(priv_p, np, new_np);
		__bfe0_overwritten_wait(priv_p, np, a_np);
		nid_log_debug ("%s: call_overwritten  new_np: %p wait np: %p", log_header, new_np, np);
	} else if (np->d_flush_overwritten) {
		/*
		 * Scenario 2>
 ...
 */
static void
bfe0_del_node(struct bfe_interface *bfe0_p, struct data_description_node *np,
	struct data_description_node *new_np)
{
	char *log_header = "bfe0_del_node";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	uint32_t page_sz = priv_p->p_pagesz;
	struct pp_page_node *pnp = np->d_page_node;
	...
	if (!np->d_flushing) {
		...
		if (__bfe0_pre_finish_flush(priv_p, pnp)) {
			__bfe0_finish_flush(priv_p, pnp);
		}
	}
...
static void
bfe0_flush_seq(struct bfe_interface *bfe0_p, uint64_t seq)
{
	char *log_header = "bfe0_flush_seq";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
...
static void
bfe0_close(struct bfe_interface *bfe0_p)
{
	char *log_header = "bfe0_close";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	struct d1an_interface *d1an_p = &priv_p->p_d1an;
	struct d2cn_interface *d2cn_p = &priv_p->p_d2cn;
	...
	free(priv_p);
	bfe0_p->bf_private = NULL;
}

static void
bfe0_end_page(struct bfe_interface *bfe0_p, struct pp_page_node *pnp)
{
	char *log_header = "bfe0_end_page";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
...
static void
bfe0_vec_start(struct bfe_interface *bfe0_p)
{
	char *log_header = "bfe0_vec_start";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	priv_p->p_vec_stat = 1;
...
static void
bfe0_vec_stop( struct bfe_interface *bfe0_p)
{
	char *log_header = "bfe0_vec_stop";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	priv_p->p_vec_stat = 0;
	nid_log_debug("%s", log_header);
}
...
static void
bfe0_get_stat(struct bfe_interface *bfe0_p, struct io_vec_stat *stat)
{
	char *log_header = "bfe0_get_stat";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
...
static void
bfe0_get_rw_stat(struct bfe_interface *bfe0_p, struct bfe_rw_stat *sp)
{
	char *log_header = "bfe0_get_rw_stat";
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	struct pp_page_node *pnp;
	struct d2c_node *tfn;
...
static uint64_t
bfe0_get_coalesce_index(struct bfe_interface *bfe0_p) {
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
...
static void
bfe0_reset_coalesce_index(struct bfe_interface *bfe0_p) {
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
...
static int
bfe0_wait_flush_page_counter(struct bfe_interface *bfe0_p, uint32_t flush_page_nwait, struct timespec *ts)
{
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	int ret;
...
static struct bfe_operations bfe0_op = {
	.bf_flush_page = bfe0_flush_page,
	.bf_right_extend_node = bfe0_right_extend_node,
	.bf_add_node = bfe0_add_node,
	.bf_left_cut_node = bfe0_left_cut_node,
	.bf_right_cut_node = bfe0_right_cut_node,
	.bf_right_cut_node_adjust = bfe0_right_cut_node_adjust,
	.bf_del_node = bfe0_del_node,
	.bf_flush_seq = bfe0_flush_seq,
	.bf_close = bfe0_close,
	.bf_end_page = bfe0_end_page,
	.bf_vec_start = bfe0_vec_start,
	.bf_vec_stop = bfe0_vec_stop,
	.bf_get_stat = bfe0_get_stat,
	.bf_get_rw_stat = bfe0_get_rw_stat,
	.bf_get_coalesce_index = bfe0_get_coalesce_index,
	.bf_reset_coalesce_index = bfe0_reset_coalesce_index,
	.bf_wait_flush_page_counter = bfe0_wait_flush_page_counter,
};

static void *
_bfe0_not_ready_thread(void *p)
{
	char *log_header = "_bfe0_not_ready_thread";
	struct bfe_interface *bfe0_p = (struct bfe_interface *)p;
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
 ...
 */
static int 
__bfe0_wait_flushing(struct bfe0_private *priv_p)
{
...
static void *
_bfe0_flush_thread_async(void *p)
{
	char *log_header = "_bfe0_flush_thread_async";
	struct bfe_interface *bfe0_p = (struct bfe_interface *)p;
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	// struct rw_interface *rw_p = priv_p->p_rw;
	struct d2c_node *tfn;
...
wait_flushing:
	if (just_wait_flushing) {
		if (__bfe0_wait_flushing(priv_p)) {
			pthread_mutex_unlock(&priv_p->p_flush_lck);
		} else {
			seq_to_flush = priv_p->p_seq_to_flush;
			priv_p->p_seq_to_flush = 0;	// we are done
			pthread_mutex_unlock(&priv_p->p_flush_lck);
			if (priv_p->p_last_seq_flushed < seq_to_flush)
 ...
 */
static void
__bfe0_overwritten_cleanup(struct bfe0_private *priv_p, struct d1a_node *fan)
{
	char *log_header = "__bfe0_overwritten_cleanup";
	struct lstn_interface *lstn_p = priv_p->p_lstn;
...
static void *
_bfe0_post_async_flush_thread(void *p)
{
	char *log_header = "_bfe0_post_async_flush_thread";
	struct bfe_interface *bfe0_p = (struct bfe_interface *)p;
	struct bfe0_private *priv_p = (struct bfe0_private *)bfe0_p->bf_private;
	struct d1an_interface *d1an_p = &priv_p->p_d1an;
	uint32_t page_sz = priv_p->p_pagesz;
	...
	while (!list_empty(&priv_p->p_flush_finished_head)) {
		...
		while (!list_empty(&fan->d1_arg.ag_coalesce_head)) {
			...
			if ((fan2 = (struct d1a_node *)dnp->d_callback_arg)) {
				...
				__bfe0_overwritten_cleanup(priv_p, fan2);
				d1an_p->d1_op->d1_put_node(d1an_p, fan2);
		...
		if (!list_empty(&fan->d1_arg.ag_overwritten_head)) {
			...
			__bfe0_overwritten_cleanup(priv_p, fan);
		}
		...
		if (!my_tfn->d2_partial_flush) {
			if (__bfe0_pre_finish_flush(priv_p, my_pnp)) {
				nid_log_debug("%s: finish flush ...", log_header);
				__bfe0_finish_flush(priv_p, my_pnp);
			}
		} else {
...
int
bfea0_initialization(struct bfe_interface *bfe0_p, struct bfe0_setup *setup0)
{
	char *log_header = "bfe0_initialization";
	struct bfe0_private *priv_p;
	...
	nid_log_info("%s: start ...", log_header);
	assert(setup);
	bfe0_p->bf_op = &bfe0_op;
	priv_p = calloc(1, sizeof(*priv_p));
	bfe0_p->bf_private = priv_p;
	...
	pthread_create(&thread_id, &attr_flush, _bfe0_not_ready_thread, (void *)bfe0_p);

	pthread_attr_init(&attr_flush);
	pthread_attr_setdetachstate(&attr_flush, PTHREAD_CREATE_DETACHED);
	pthread_create(&thread_id, &attr_flush, _bfe0_flush_thread_async, (void *)bfe0_p);

	pthread_attr_init(&attr_post_flush);
	pthread_attr_setdetachstate(&attr_post_flush, PTHREAD_CREATE_DETACHED);
	pthread_create(&thread_id, &attr_post_flush, _bfe0_post_async_flush_thread, (void *)bfe0_p);
