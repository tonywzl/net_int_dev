...
TEST_DATA_FILE_OUT=/tmp/testdatabk_out$1$2$3
BS=$1
CNT=$2
OFFSET=$3
...
time dd if=$TEST_DATA_FILE of=/dev/nid1 bs=$BS count=$CNT seek=$OFFSET iflag=sync oflag=sync

>$TEST_DATA_FILE_OUT

echo ""
echo "Do dd read on device nid1 ..."
time dd if=/dev/nid1 of=$TEST_DATA_FILE_OUT bs=$BS count=$CNT skip=$OFFSET iflag=sync oflag=sync
